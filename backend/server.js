...
app.post("/note/addrecent/:id", (req, res) => {
	db.query(
		"insert into recent (note_id, date) values(?,?)",
		[req.params.id, getDateModified()],
		(err, result) => {
			if (err) {
				console.log(err);
			} else {
...
app.get("/search/:term", (req, res) => {
	db.query(
		"SELECT f.name as folder, n.*, n.name as label, concat('note') as type from notes n left join folders f on n.folder_id = f.id where n.name like concat('%', ?, '%') or n.text like concat('%', ?, '%' )",
